...
describe('S7.2 Scoped recall accuracy', () => {
  let userId: string
  let threadId: string

  beforeAll(async () => {
    const email = `recall-acc-${Date.now()}@example.com`
    const user = await prisma.user.create({ data: { email } })
    userId = user.id
    process.env.TEST_USER_ID = userId
    const t = await prisma.thread.create({ data: { userId, title: 'Recall Acc', activeModel: 'openai:gpt-4o' } })
    threadId = t.id
  ...
  it('with only Work ON, usedScopes === ["work"]', async () => {
    ...
    const req = new Request('http://local/api/messages', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ threadId, content: 'Work-related question' })
    })
    ...
    expect(payload.usedScopes).toEqual(['work'])
  })
})
